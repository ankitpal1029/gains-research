...
library TradingInteractionsUtils {
    ...
    function updateMarketOrdersTimeoutBlocks(uint16 _valueBlocks) internal {
        ...
        emit ITradingInteractionsUtils.MarketOrdersTimeoutBlocksUpdated(
            _valueBlocks
        );
    }
    ...
    function updateByPassTriggerLink(
        ...
        ITradingInteractions.TradingInteractionsStorage
            storage s = _getStorage();
        ...
        for (uint256 i = 0; i < _users.length; i++) {
            ...
            emit ITradingInteractionsUtils.ByPassTriggerLinkUpdated(
                user,
                value
            );
        }
    ...
    function updateOpenOrder(
        ...
        address sender = _msgSender();
        ITradingStorage.Trade memory o = _getMultiCollatDiamond().getTrade(
            sender,
            _index
        );
    ...
    function updateLeverageNative(
        ...
        uint8 collateralIndex = _getMultiCollatDiamond()
            .getTrade(_msgSender(), _index)
            .collateralIndex;
        uint120 nativeBalance = _wrapNativeToken(collateralIndex);

        // Perform updateLeverage request
        uint120 unusedBalance = _updateLeverage(
            _index,
            _newLeverage,
            true,
            nativeBalance
    ...
    function increasePositionSizeNative(
        ...
        _collateralDelta = _wrapNativeToken(
    ...
    function triggerOrder(uint256 _packed) internal notDelegatedAction {
        (uint8 _orderType, address _trader, uint32 _index) = _packed
            .unpackTriggerOrder();

        ITradingStorage.PendingOrderType orderType = ITradingStorage
            .PendingOrderType(_orderType);

        if (ConstantsUtils.isOrderTypeMarket(orderType))
            revert IGeneralErrors.WrongOrderType();

        bool isOpenLimit = orderType ==
            ITradingStorage.PendingOrderType.LIMIT_OPEN ||
            orderType == ITradingStorage.PendingOrderType.STOP_OPEN;

        ITradingStorage.TradingActivated activated = _getMultiCollatDiamond()
            .getTradingActivated();
        if (
            (isOpenLimit &&
                activated != ITradingStorage.TradingActivated.ACTIVATED) ||
            (!isOpenLimit &&
                activated == ITradingStorage.TradingActivated.PAUSED)
        ) {
        ...
        uint256 positionSizeCollateral = TradingCommonUtils
            .getPositionSizeCollateral(t.collateralAmount, t.leverage);

        if (isOpenLimit) {
            (uint256 priceImpactP, ) = TradingCommonUtils
        ...
        if (!byPassesLinkCost) {
            ...
            IERC20(_getMultiCollatDiamond().getChainlinkToken())
                .safeTransferFrom(
                    sender,
                    address(this),
        ...
        ITradingStorage.Id memory orderId = _getPriceTriggerOrder(
            t,
            pendingOrder,
            byPassesLinkCost ? 0 : positionSizeCollateral
        );
    ...
    function cancelOrderAfterTimeout(
        ...
        if (
            ChainUtils.getBlockNumber() <
            order.createdBlock + _getStorage().marketOrdersTimeoutBlocks
    ...
    function _openTrade(
        ...
        if (_trade.tradeType != ITradingStorage.TradeType.TRADE) {
            ...
            ITradingStorage.Id memory orderId = _getMultiCollatDiamond()
                .getPrice(
                    _trade.collateralIndex,
                    _trade.pairIndex,
                    pendingOrder,
        ...
        if (_referrer != address(0)) {
            _getMultiCollatDiamond().registerPotentialReferrer(
                sender,
                _referrer
            );
        }
    ...
    function _wrapNativeToken(
        ...
        emit ITradingInteractionsUtils.NativeTokenWrapped(
            msg.sender,
            nativeValue
        );
