...
library DecreasePositionSizeUtils {
    ...
    function validateRequest(
        ...
        uint256 closingFeesCollateral = TradingCommonUtils.getTotalTradeFeesCollateral(
            _trade.collateralIndex, _trade.user, _trade.pairIndex, positionSizeCollateralDelta
        );
    ...
    function prepareCallbackValues(
        ...
        (, values.priceAfterImpact,) = TradingCommonUtils.getTradeClosingPriceImpact(
            ITradingCommonUtils.TradePriceImpactInput(
                _existingTrade, _answer.price, _answer.spreadP, values.positionSizeCollateralDelta
            )
        );
        ...
        values.existingPnlCollateral = (
            TradingCommonUtils.getPnlPercent(
                _existingTrade.openPrice, uint64(values.priceAfterImpact), _existingTrade.long, _existingTrade.leverage
        ...
        values.availableCollateralInDiamond =
            int256(uint256(_partialTrade.collateralAmount)) - int256(values.closingFeeCollateral);
    ...
    function validateCallback(
        ...
        uint256 maxSlippageP =
            _getMultiCollatDiamond().getTradeInfo(_existingTrade.user, _existingTrade.index).maxSlippageP;
        uint256 maxSlippage = (
            expectedPrice * (maxSlippageP > 0 ? maxSlippageP : ConstantsUtils.DEFAULT_MAX_CLOSING_SLIPPAGE_P)
        ) / 100 / 1e3;
        ...
        return (
            ...
            : (
                _existingTrade.long
                    ...
                    : _values.priceAfterImpact > expectedPrice + maxSlippage
            ) ? ITradingCallbacks.CancelReason.SLIPPAGE : ITradingCallbacks.CancelReason.NONE;
    }
        ...
        internal
    {
        if (_cancelReason != ITradingCallbacks.CancelReason.NO_TRADE) {
            ...
            TradingCommonUtils.distributeExactGovFeeCollateral(
                _existingTrade.collateralIndex, _existingTrade.user, govFeeCollateral
            );
