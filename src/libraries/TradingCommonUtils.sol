...
library TradingCommonUtils {
    ...
    function getMarketExecutionPrice(
        ...
        if (
            !_open &&
            _contractsVersion == ITradingStorage.ContractsVersion.BEFORE_V9_2
        ) {
            return _price;
        }
    ...
    function convertCollateralToGns(
        ...
        return ((_collateralAmount *
            _collateralPrecisionDelta *
            ConstantsUtils.P_10) / _gnsPriceCollateral);
    }
    ...
    function getTradeValueCollateral(
        ...
        public
        view
        returns (uint256 valueCollateral, uint256 borrowingFeesCollateral)
    {
    ...
    function getTradeOpeningPriceImpact(
        ...
        (priceImpactP, priceAfterImpact) = _getMultiCollatDiamond()
            .getTradePriceImpact(
                trade.user,
        ...
        )
    {
        ...
        ITradingStorage.TradeInfo memory tradeInfo = _getMultiCollatDiamond()
            .getTradeInfo(trade.user, trade.index);
        ...
        uint256 positionSizeUsd = _getMultiCollatDiamond()
        ...
        (, uint256 priceNoProtectionFactor) = _getMultiCollatDiamond()
            .getTradePriceImpact(
                trade.user,
                priceAfterSpread,
                trade.pairIndex,
        ...
        return
            _getMultiCollatDiamond().calculateFeeAmount(
                ...
                (getPositionSizeCollateralBasis(
                    ...
                    _getMultiCollatDiamond().pairTotalPositionSizeFeeP(
                        _pairIndex
                    )) /
                    ConstantsUtils.P_10 /
                    100
    ...
    function getTradeFeesCollateral(
        ...
        IPairsStorage.GlobalTradeFeeParams
            memory feeParams = _getMultiCollatDiamond()
                .getGlobalTradeFeeParams();

        // 1. Calculate total fee
        if (_orderType == ITradingStorage.PendingOrderType.LIQ_CLOSE) {
            uint256 totalLiqCollateralFeeP = _getMultiCollatDiamond()
                .pairTotalLiqCollateralFeeP(_pairIndex);
        ...
        address referrer = _getMultiCollatDiamond().getTraderActiveReferrer(
            _trader
        );
        ...
        if (referrer != address(0)) {
            ...
            uint256 referralFeeP = referralFeeP_override > 0
                ? referralFeeP_override
                : feeParams.referralFeeP;
            uint256 referralFeeRaw = (tradeFees.totalFeeCollateral *
                referralFeeP) /
                1e3 /
                100;
            ...
            tradeFees.referralFeeCollateral =
                (referralFeeRaw *
                    _getMultiCollatDiamond().getReferrerFeeProgressP(
                        referrer
                    )) /
                ConstantsUtils.P_10 /
                100;
        ...
        uint256 totalFeeCollateral = tradeFees.totalFeeCollateral -
            tradeFees.referralFeeCollateral;

        // 4.1 Gov fee
        tradeFees.govFeeCollateral = ((totalFeeCollateral * feeParams.govFeeP) /
            1e3 /
            100);

        // 4.2 Trigger fee
        uint256 triggerOrderFeeCollateral = (totalFeeCollateral *
            feeParams.triggerOrderFeeP) /
            1e3 /
            100;
        tradeFees.triggerOrderFeeCollateral = ConstantsUtils.isOrderTypeMarket(
            _orderType
        )
            ? 0
        ...
        uint256 missingTriggerOrderFeeCollateral = triggerOrderFeeCollateral -
        ...
        // 4.4 gToken fee
        tradeFees.gTokenFeeCollateral =
            (totalFeeCollateral * feeParams.gTokenFeeP) /
            1e3 /
            100;
    }
    ...
    function getMinGovFeeCollateral(
        ...
        return
            (totalFeeCollateral *
                _getMultiCollatDiamond().getGlobalTradeFeeParams().govFeeP) /
            1e3 /
            100;
    }
     ...
     */
    function revertIfTradeHasPendingMarketOrder(
        address _user,
        uint32 _index
    ) public view {
        ITradingStorage.PendingOrderType[5]
            memory pendingOrderTypes = ConstantsUtils.getMarketOrderTypes();
        ...
        for (uint256 i; i < pendingOrderTypes.length; ++i) {
            ...
            if (
                ...
                revert ITradingInteractionsUtils.ConflictingPendingOrder(
                    orderType
                );
        }
    }
    ...
    function transferCollateralFrom(
        ...
        if (_amountCollateral > 0) {
            _getMultiCollatDiamond()
                ...
                .collateral
                .transferFrom(_from, address(this), _amountCollateral);
        }
    }
    ...
    function transferCollateralTo(
        ...
        if (_amountCollateral > 0) {
            address collateral = _getMultiCollatDiamond()
                .getCollateral(_collateralIndex)
                .collateral;
    ...
    function transferGnsFrom(address _from, uint256 _amountGns) internal {
        if (_amountGns > 0) {
            AddressStoreUtils.getAddresses().gns.transferFrom(
                _from,
                address(this),
                _amountGns
            );
    ...
    function handleTradePnl(
        ...
        if (_collateralSentToTrader > _availableCollateralInDiamond) {
            ...
            int256 collateralFromGToken = _collateralSentToTrader -
    ...
    function updateFeeTierPoints(
        ...
        uint256 usdNormalizedPositionSize = _getMultiCollatDiamond()
            .getUsdNormalizedValue(_collateralIndex, _positionSizeCollateral);
        _getMultiCollatDiamond().updateTraderPoints(
            _trader,
    ...
    function distributeVaultFeeCollateral(
        ...
        emit ITradingCommonUtils.GTokenFeeCharged(
            _trader,
            _collateralIndex,
            _valueCollateral
        );
    ...
    function distributeExactGovFeeCollateral(
        ...
        TradingCallbacksUtils._getStorage().pendingGovFees[
                _collateralIndex
            ] += _govFeeCollateral;
        emit ITradingCommonUtils.GovFeeCharged(
            _trader,
            _collateralIndex,
            _govFeeCollateral
        );
     ...
     */
    function distributeGnsOtcFeeCollateral(
        uint8 _collateralIndex,
        address _trader,
        uint256 _amountCollateral
    ) public {
        _getMultiCollatDiamond().addOtcCollateralBalance(
            _collateralIndex,
            _amountCollateral
        );
        emit ITradingCommonUtils.GnsOtcFeeCharged(
            _trader,
            _collateralIndex,
            _amountCollateral
        );
    ...
    function processFees(
        ...
        uint128 collateralPrecisionDelta = _getMultiCollatDiamond()
            ...
            .precisionDelta;
        uint256 gnsPriceCollateral = _getMultiCollatDiamond()
            .getGnsPriceCollateralIndex(_trade.collateralIndex);
    ...
    function distributeReferralFeeCollateral(
        ...
        _getMultiCollatDiamond().distributeReferralReward(
            ...
            _getMultiCollatDiamond().getGnsPriceUsd(
                _collateralIndex,
                _gnsPriceCollateral
            )
        );
