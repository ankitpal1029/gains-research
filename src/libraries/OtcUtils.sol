...
library OtcUtils {
    ...
    function updateOtcConfig(IOtc.OtcConfig memory _config) internal {
        // `OtcConfig.gnsTreasury` is deprecated, updated config must always have address(0)
        if (_config.gnsTreasury != address(0))
            revert IGeneralErrors.WrongParams();

        if (
            _config.treasuryShareP +
                _config.stakingShareP +
                _config.burnShareP !=
    ...
    function addOtcCollateralBalance(
        ...
        uint256 newBalanceCollateral = s.collateralBalances[_collateralIndex] +
            _collateralAmount;
        s.collateralBalances[_collateralIndex] = newBalanceCollateral;

        emit IOtcUtils.OtcBalanceUpdated(
            _collateralIndex,
            newBalanceCollateral
        );
    }
    ...
    function sellGnsForCollateral(
        ...
        IOtc.OtcStorage storage s = _getStorage();
        bool isCollateralNotGns = !_getMultiCollatDiamond().isCollateralGns(
            _collateralIndex
        );

        uint256 availableCollateral = s.collateralBalances[_collateralIndex];
        uint256 gnsPriceCollateral = _getOtcRate(
            _collateralIndex,
            isCollateralNotGns
        );
        ...
        if (_collateralAmount > availableCollateral)
            revert IGeneralErrors.InsufficientBalance();
        if (gnsAmount < MIN_GNS_WEI_IN) revert IGeneralErrors.BelowMin();

        // 2. Receive GNS from caller (if collateral is not GNS)
        if (isCollateralNotGns)
        ...
        // 5. Transfer collateral to caller (if collateral is not GNS)
        if (isCollateralNotGns)
    ...
    /**
     * @dev Returns OTC rate (price + premium, when `applyPremium` is true) of GNS in collateral (1e10)
     * @param _collateralIndex collateral index
     * @param _applyPremium whether premium should be applied. At the moment, premium is only applied for non-GNS collaterals
     */
    function _getOtcRate(
        uint8 _collateralIndex,
        bool _applyPremium
    ) internal view returns (uint256) {
        uint256 baseRate = _getMultiCollatDiamond().getGnsPriceCollateralIndex(
            _collateralIndex
        );
        uint64 premiumP = _applyPremium ? _getStorage().otcConfig.premiumP : 0;

        return
            premiumP > 0
                ? (baseRate +
                    ((baseRate * premiumP) / 100 / ConstantsUtils.P_10))
                : baseRate;
    }
        ...
        )
    {
        ...
        treasuryAmountGns =
            (_gnsAmount * config.treasuryShareP) /
            100 /
            ConstantsUtils.P_10;
        stakingAmountGns =
            (_gnsAmount * config.stakingShareP) /
            100 /
            ConstantsUtils.P_10;
        burnAmountGns =
            (_gnsAmount * config.burnShareP) /
            100 /
            ConstantsUtils.P_10;
    }
    ...
    function _distributeTreasuryGns(uint256 _gnsAmount) internal {
        TradingCommonUtils.transferGnsTo(
            AddressStoreUtils.getAddresses().treasury,
            _gnsAmount
        );
    }
    ...
    function _distributeStakingGns(uint256 _gnsAmount) internal {
        IAddressStore.Addresses storage addresses = AddressStoreUtils
            .getAddresses();
        IGNSStaking(addresses.gnsStaking).distributeReward(
            addresses.gns,
            _gnsAmount
        );
    }
    ...
    function _burnGns(uint256 _gnsAmount) internal {
        IERC20(AddressStoreUtils.getAddresses().gns).burn(
            address(this),
            _gnsAmount
        );
    }
